                  ...
                  <div class="card">
                    <div class="card-close">
                      <div class="dropdown">
                        ...
                        <div aria-labelledby="closeCard2" class="dropdown-menu dropdown-menu-right has-shadow"><a href="#" class="dropdown-item remove"> <i class="fa fa-times"></i>离开</a><a href="#" class="dropdown-item edit"> <i class="fa fa-gear"></i>关闭</a></div>
                      </div>
                    </div>
                    <div class="card-header d-flex align-items-center">
                      <h3 class="h4">历史比赛信息</h3>
                    </div>
                    <div class="card-body">
                      <div class="table-responsive">
                        <table class="table">
                          ...
                          <tbody>
                          <c:forEach items="${allMatch }" var="t" varStatus="i">
                          <tr>
                              <th scope="row">${i.index+1 }</th>
						<form action="${path }/updateMatchServlet?mid=${t.mid}" method="post">
                        <td><input style="color: #3DB3E4;" value="${t.name }" name="name"></td>
                              <td><input value="${t.starttime }" name="starttime"></td>
                              <td><input value="${t.uptime }" name="uptime"></td>
                              <c:if test="${t.state==0 }">
                              <td><input style="color: #777777;" value="未开始" readonly="readonly"></td>
                              </c:if>
                               <c:if test="${t.state==1 }">
                              <td><input style="color: #68FF6B;" value="进行中" readonly="readonly"></td>
                              </c:if>
                               <c:if test="${t.state==2 }">
                              <td><input style="color: #FF0004;" value="已结束" readonly="readonly"></td>
                              </c:if>
						
							<td><button type="submit" class="btn btn-primary">修改</button></td>
							 <c:if test="${t.state==2 }">
                             <td><input type="button" class="btn btn-primary" value="已下线" style="width: 60px;padding-right: 5px"></td>
                              </c:if>
                               <c:if test="${t.state!=2 }">
                             <td><input type="button" class="btn btn-primary" value="下线" style="width: 60px"onclick="window.location.href='${path}/XXMatchServlet?mid=${t.mid }'"></td>
                              </c:if>
                              
							<td><input type="button" class="btn btn-primary" value="删除" style="width: 60px" onclick="window.location.href='${path}/delMatchServlet?mid=${t.mid }'"></td>
                      </form>
                            </tr>
                          </c:forEach>
                          </tbody>
