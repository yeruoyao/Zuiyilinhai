...
<!--                    <li><a href="${path }/backstage2/forms.jsp"> <i class="icon-padnote"></i>网站信息管理 </a></li>-->
        ...
        <div class="content-inner">
          ...
          <div class="breadcrumb-holder container-fluid">
            <ul class="breadcrumb">
              <li class="breadcrumb-item"><a href="${path }/backstage2/index.jsp">主页</a></li>
                    ...
                    <div class="card-body">
                      <div class="table-responsive">
                        <table class="table">
                         ...
                         <tbody>
													<c:forEach items="${findAllTbUser3 }" var="t" varStatus="i">
														<tr>
															<th scope="row">${i.index+1 }</th>
															<form action="${path }/UpdateTbUserServlet3" method="post">
															<td style="display: none;"><input value="${t.id }"
																	name="id"></td>
																<td><input value="${t.username }" name="username"></td>
																<td><input value="${t.password }" name="password"></td>
																<td><input value="${t.email }" name="email"></td>
																<td><input value="${t.score }" name="score"></td>
																<c:if test="${t.state2==1 }">
																	<td><input value="正常" name="state2"
																		readonly="readonly"></td>
																</c:if>
																<c:if test="${t.state2!=1 }">
																	<td><input value="禁言" name="state2"
																		readonly="readonly"></td>
																</c:if>

																<td><input value="${t.state }" name="state"
																	readonly="readonly"></td>
																<td><button type="submit" class="btn btn-primary">修改</button></td>
																<c:if test="${t.state2==1 }">
																	<td><input type="button" value="禁言"
																		class="btn btn-primary" style="width: 60px"
																		onclick="window.location.href='${path}/updateState2Servlet3?id=${t.id }&state2=1'"></td>
																</c:if>
																<c:if test="${t.state2!=1 }">
																	<td><input type="button" value="正常"
																		class="btn btn-primary" style="width: 60px"
																		onclick="window.location.href='${path}/updateState2Servlet3?id=${t.id }&state2=0'"></td>
																</c:if>

																<td><input type="button" value="注销"
																	class="btn btn-primary" style="width: 60px"
																	onclick="window.location.href='${path}/delTbUserServlet3?id=${t.id }'"></td>
															</form>
															</form>
														</tr>
