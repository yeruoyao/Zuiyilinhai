					...
					<ul class="list-unstyled">
						...
						<li>
							...
							<ul id="exampledropdownDropdown6" class="collapse list-unstyled ">
								...
								<li>
									<a href="<%=base %>WishController">特色小镇</a>
								</li>
							</ul>
						</li>
										...
										<div class="card-close">
											<div class="dropdown">
												...
												<div aria-labelledby="closeCard5" class="dropdown-menu dropdown-menu-right has-shadow">
													<a href="#" class="dropdown-item remove"> <i class="fa fa-times"></i>离开</a>
													<a href="#" class="dropdown-item edit"> <i class="fa fa-gear"></i>关闭</a>
												</div>
											</div>
										</div>
										<div class="card-header d-flex align-items-center">
											<h3 class="h4">非遗传人-板块内容编辑</h3>
										</div>
										<div class="card-body">
											<div class="table-responsive">
												<table class="table">
													...
													<tbody>
															<% 
																List<Fycr> list=(List)request.getAttribute("list");
															for(int i=0;i<list.size();i++){
															%>
														<tr> 
													
																<th scope="row"><%=list.get(i).getFycrid() %></th>
																<td><%=list.get(i).getFycrtitle() %></td>
																<td>
																<div   class=ellipsis_div     
 								 style="width:170px;" title="somelongtext">
								  <%=list.get(i).getFycrtext() %></div>
																	
																</td>
																<td><%=list.get(i).getFycrtime() %></td>
															
																<form action="<%=base %>FycrUpdateController1" method="post">
	                              									<input name="fycrid" value=<%=list.get(i).getFycrid() %> type="hidden"/>
	                              									<input name="fycrtitle" value=<%=list.get(i).getFycrtitle() %> type="hidden">
	                              									<input name="fycrtext" value=<%=list.get(i).getFycrtext() %> type="hidden">
	                              									
																	<td><button type="submit" class="btn btn-primary">更新</button></td>
																</form>
																
																<form action="<%=base%>FycrController" method="post">
								                              	 	<input name="fycrid" value="<%=list.get(i).getFycrid() %>" type="hidden"/>
										   ...
										   <form action="<%=base %>FycrAddController" >
                     						  	<button id="regidter" type="submit" name="" class="btn btn-primary">新增</button>
                     						</form>
										</div>
									</div>
								</div>
